...
from src.core.models import SessionNotFoundError, ResourceTooLargeError
...
class SessionVault:
    ...
    def create(self, entities: list[Entity]) -> str:
        with self._lock:
            ...
            if size > self.max_bytes: raise ValueError("session mapping exceeds byte budget")
            while self.sessions and (len(self.sessions) >= self.max_sessions or self._bytes + size > self.max_bytes):
                old_sid = next(iter(self.sessions))
                old = self.sessions.pop(old_sid)
                self._bytes -= old.size_bytes
            sid = str(uuid.uuid4())
            self.sessions[sid] = Session(mappings, datetime.now(timezone.utc), size)
            self._bytes += size
            self.stats["total_anonymizations"] += 1; self.stats["total_pii_entities"] += len(entities)
            for e in entities:
                self.stats["entities_by_type"][e.entity_type] = self.stats["entities_by_type"].get(e.entity_type, 0) + 1
                self.stats["blocked_pii_types"][e.entity_type] = self.stats["blocked_pii_types"].get(e.entity_type, 0) + 1
            return sid
