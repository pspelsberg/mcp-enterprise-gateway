...
class _NoRedirect(HTTPRedirectHandler):
    """Never forward a bearer token to an attacker-controlled redirect target."""
...
def _fetch_stats() -> dict:
    ...
    with _open_no_redirect(request, timeout=2) as response:
...
class _Handler(BaseHTTPRequestHandler):
    ...
    def _authorized(self) -> bool:
        ...
        return bool(token and len(token) >= 32 and hmac.compare_digest(authorization, expected))
    def do_GET(self):
        if self.path == "/":
            token = os.getenv("AUDIT_STUDIO_TOKEN")
...
def main():  # pragma: no cover
    ...
    token = os.getenv("AUDIT_STUDIO_TOKEN")
    if not token or len(token) < 32:
        parser.error("AUDIT_STUDIO_TOKEN must contain at least 32 characters")
